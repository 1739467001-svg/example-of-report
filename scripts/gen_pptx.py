...
def fit_card(sl,path,cx,top,boxh,cap):
    iw,ih=Image.open(path).size; ar=iw/ih; h=boxh; w=h*ar; pad=0.14
    rect(sl,cx-w/2-pad,top-pad,w+2*pad,h+2*pad,fill=CARDBG,line=LINE,lw=0.75,radius=0.04,shadow=True)
    sl.shapes.add_picture(path,Inches(cx-w/2),Inches(top),Inches(w),Inches(h))
    text(sl,cx-2.4,top+h+0.22,4.8,0.35,cap,12,GRAY,align=PP_ALIGN.CENTER)
...
def bullets(sl,l,t,w,items,size=15,gap=0.62,dot=BLUE):
    ...
    for it in items:
        ...
        text(sl,l+0.30,y-0.04,w-0.30,gap,it,size,TEXT,ls=1.15); y+=gap
...
def placeholder(sl,l,t,w,h,label):
    ...
    text(sl,l,t+h/2,w,0.4,"建议替换为真实截图",17,RGBColor(0x5C,0x6B,0x82),True,PP_ALIGN.CENTER)
    text(sl,l,t+h/2+0.45,w,0.4,label,13,PHT,False,PP_ALIGN.CENTER)

def fdetail(n,tab,title,sub,items,img=None,cap=""):
    sl=slide(); bg(sl); header(sl,tab,title,sub)
    bullets(sl,0.7,2.55,5.0,items,size=16,gap=0.78)
    if img:
        bx,bt,bw,bh=6.3,2.45,6.4,4.0
        rect(sl,bx,bt,bw,bh,fill=CARDBG,line=LINE,lw=0.75,radius=0.04,shadow=True)
        fit_img(sl,img,bx+0.18,bt+0.18,bw-0.36,bh-0.6)
        text(sl,bx,bt+bh-0.36,bw,0.3,cap,12,GRAY,align=PP_ALIGN.CENTER)
    else:
        placeholder(sl,6.1,2.5,6.6,3.9,cap)
...
# 10 REAL EVAL FORM
def s_evalform():
    sl=slide(); bg(sl); header(sl,"04 核心功能 · 功能③","真实评价表单","四大维度 + 互斥单选 + 定性文字，完整复刻学校《课程评价表》")
    fit_card(sl,f"{S}/evalform1.png",3.7,2.45,3.7,"一、教师风范 / 二、学生状态")
    fit_card(sl,f"{S}/evalform2.png",9.2,2.45,3.7,"三、课程内容（含 4.1/4.2 互斥）/ 四、教学过程 / 定性")
    footer(sl,10)

# 11 EVAL RECORD
def s_evalrecord():
    sl=slide(); bg(sl); header(sl,"04 核心功能 · 功能③","评价记录与详情","评价详情清晰呈现各维度得分、综合评分与星级")
    rect(sl,2.0,2.4,9.33,3.95,fill=CARDBG,line=LINE,lw=0.75,radius=0.04,shadow=True)
    fit_img(sl,f"{S}/eval-record.png",2.2,2.55,8.93,3.5)
    text(sl,2.0,6.05,9.33,0.35,"课程评价记录详情页（真实界面）",12,GRAY,align=PP_ALIGN.CENTER)
    footer(sl,11)

# 12 WORKBENCH + RADAR
def s_dashboard():
    sl=slide(); bg(sl); header(sl,"04 核心功能 · 功能④","工作台 · 数据看板","关键指标一屏总览（待听课/已完成评价/通知/全校课程），评价结果多维可视化")
    rect(sl,0.62,2.4,7.7,3.95,fill=CARDBG,line=LINE,lw=0.75,radius=0.04,shadow=True)
    fit_img(sl,f"{S}/workbench.png",0.8,2.55,7.34,3.5)
    text(sl,0.62,6.1,7.7,0.35,"督导工作台 · 数据看板（真实界面）",12,GRAY,align=PP_ALIGN.CENTER)
...
# 15 LOGIN + USERMENU
def s_login():
    sl=slide(); bg(sl); header(sl,"04 核心功能 · 功能⑥","工号登录与用户管理","工号为唯一凭证 · 初始密码即工号 · 可自助改密 · 角色权限与数据隔离")
    rect(sl,0.62,2.4,8.7,3.95,fill=CARDBG,line=LINE,lw=0.75,radius=0.04,shadow=True)
    fit_img(sl,f"{S}/login.png",0.8,2.55,8.34,3.5)
    text(sl,0.62,6.1,8.7,0.35,"工号登录页（真实界面）",12,GRAY,align=PP_ALIGN.CENTER)
    rect(sl,9.6,2.4,3.1,3.95,fill=CARDBG,line=LINE,lw=0.75,radius=0.04,shadow=True)
    fit_img(sl,f"{S}/usermenu.png",9.78,2.6,2.74,3.2)
    text(sl,9.6,6.1,3.1,0.35,"个人菜单 · 修改密码 / 退出",12,GRAY,align=PP_ALIGN.CENTER)
    footer(sl,15)
...
def s_ach():
    ...
    ach=[("真实数据驱动","1,431 门课 · 22 学院 · 2 校区 · 588 教师 · 58 用户"),
...
fdetail(7,"04 核心功能 · 功能①","全校课程多维筛选","按多维度精确定位目标课程，与原始课表完全一致",
        ["全校 1,431 门课程在线检索，告别翻纸质课表","支持 周次 / 星期 / 学院 / 校区 / 教师 多维组合筛选",
         "筛选逻辑与原课表一致（如“会计学院 + 第 3 周”精确联动）","督导专家可快速锁定“本周哪天、哪个学院、哪位老师”有课"],
        img=f"{S}/courses.png",cap="全校课程 · 多维筛选（真实界面）")
fdetail(8,"04 核心功能 · 功能②","听课计划 · 一键加入待办","在课程列表点“听课”即可加入计划，并可指定听课周次",
        ["在全校课程列表一键“听课”，加入听课计划（待办）","可指定计划听课周次（或“不指定周次”）",
         "听课计划支持日历视图（节次 × 星期网格）按周查看","待听课 / 已评价 / 已取消 三状态区分，课前自动提醒"],
        img=f"{S}/listenplan.png",cap="加入听课计划（真实界面）")
s_image(9,"04 核心功能 · 功能③","标准化课程评价体系","完全复刻学校现行《课程评价表》：20 项定量 + 定性，系统自动计算综合评分",f"{D}/eval-system.png")
s_evalform(); s_evalrecord(); s_dashboard(); s_stats(); s_notify(); s_login()
s_image(16,"05 业务闭环","督导业务闭环","从找课到改进，串成一条可沉淀、可统计、可追溯的数字化链路",f"{D}/business-loop.png")
s_image(17,"05 技术架构","系统架构（简要）","前后端分离 · 数据驱动 · 真实课表自动解析建库 · 已部署阿里云上线运行",f"{D}/architecture.png")
s_ach(); s_dev(); s_value(); s_roadmap()
s_qa(22,"07 答辩 Q&A","常见问题预演（一）","建设与数据",[
...
s_qa(23,"07 答辩 Q&A","常见问题预演（二）","应用与推广",[
